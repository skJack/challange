...
import roc
import models
from dataloader import load_cisia_csfa
from losses import *
...
parser.add_argument('--protoal',default='4@1',type=str,help='the protoal of the data')
parser.add_argument('--pretraned',default=False,type=bool,help='use pretraned model or not')
parser.add_argument('--path',default="'/media/sdc/datasets/Face-anti-spoofing/CASIA-CeFA/phase1'",type=str)
best_prec1 = 0
...
def main():
    ...
    if args.input_size != 224 or args.image_size != 256:
        ...
        image_size = 114
        input_size = 112
    logger.Print("Input image size: {}, test size: {}".format(image_size, input_size))
    ...
    if args.resume:
        ...
        if os.path.isfile(args.resume):
            ...
            logger.Print("=> loaded checkpoint '{}' (epoch {})"
    ...
    train_loader,val_loader = load_cisia_csfa(root = args.train_path,protoal = args.protoal,img_size=img_size,train_batch = args.batch_size)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if is_best:
            logger.Print('epoch: {} The best is {} last best is {}'.format(epoch,prec1,best_prec1))
...
def train(train_loader, model, criterion, optimizer, epoch):
    ...
    for i, data in enumerate(train_loader):
        ...
        rgb_img,depth_img,ir_img,hsv_img,YCbCr_img,label = data[0],data[1],data[2],data[3],data[4],data[5]
...
def validate(val_loader, model, criterion,epoch):
    ...
    with torch.no_grad():
        for i, data in enumerate(val_loader):
            with torch.no_grad():
                rgb_img,depth_img,ir_img,hsv_img,YCbCr_img,label,dirs = data[0],data[1],data[2],data[3],data[4],data[5],data[6]
                rgb_var = Variable(rgb_img).float().to(device)
                depth_var = Variable(depth_img).float().to(device)
                ir_var = Variable(ir_img).float().to(device)
                hsv_img_var = Variable(hsv_img).float().to(device)
                ...
                for i_batch in range(preds.shape[0]):
                        ...
                        if args.val_save:
                            ...
                            rgb_dir = dirs[i_batch].replace(os.getcwd() + '/data/','')
    ...
    auc = metric[2]
    xy_dic = metric[3]
    with open(log_path+'/val_result_{}_{}.txt'.format(time_stp,args.arch),'a+') as f_result:
...
def accuracy(output, target, topk=(1,)):
    ...
    _, pred = output.topk(maxk, 1, True, True)#取出前两个
    pred = pred.t()
...
if __name__ == '__main__':
    ...
    for k, v in config['common'].items():
        setattr(args, k, v)
    log_path = f'./logs/{args.protoal}_{args.arch}/'
    print(log_path)
